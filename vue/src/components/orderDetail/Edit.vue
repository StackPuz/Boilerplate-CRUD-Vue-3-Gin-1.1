        ...
        <form method="post" @submit.prevent="edit()">
          <div class="row">
            <div class="mb-3 col-md-6 col-lg-4">
              ...
              <input readonly id="order_detail_order_id" name="OrderId" class="form-control form-control-sm" v-model="orderDetail.OrderId" type="number" required />
              <span v-if="errors.OrderId" class="text-danger">{{errors.OrderId}}</span>
            </div>
            <div class="mb-3 col-md-6 col-lg-4">
              <label class="form-label" for="order_detail_no">No</label>
              <input readonly id="order_detail_no" name="No" class="form-control form-control-sm" v-model="orderDetail.No" type="number" required />
              <span v-if="errors.No" class="text-danger">{{errors.No}}</span>
            </div>
            <div class="mb-3 col-md-6 col-lg-4">
              <label class="form-label" for="order_detail_product_id">Product</label>
              <select id="order_detail_product_id" name="ProductId" class="form-select form-select-sm" v-model="orderDetail.ProductId" required>
                <option v-for="product in products" :key="product" :value="product.Id" :selected="orderDetail.ProductId == product.Id">{{product.Name}}</option>
              </select>
